...
async function createHomepageOG() {
  const logoPath = path.join(process.cwd(), 'public', 'ADL99-LOGO.jpg');
  const outputPath = path.join(process.cwd(), 'public', 'opengraph-image.png');
  ...
  try {
    ...
    console.log(`✅ Homepage OG image created: ${outputPath}`);
    ...
    console.error('❌ Error creating homepage OG image:', error);
    process.exit(1);
  }
}
